--[[
	...
	Roblibwaita: A Roblox User Infterface Libary.
	By Blukez
  Edited by EnterpriseExperience (AKA, Anonymous Exploiting on YT)

	This UI Libary is VERY heavily inspired by the GNOME project (https://gnome.org).
]]--
...
function rolibwaita:NewWindow(WindowOptions)
	...
	local firstTab = true
	local window = {}
	function window:NewTab(TabOptions) -- tab
		...
		if TabOptions.Image ~= nil then
			tabButtonClone.Icon.Image = TabOptions.Image
		end
		...
		tabButtonClone.Hitbox.MouseButton1Up:Connect(function()
			...
			if not tabClone.Visible then 
				...
				tabTitle.Text = tabName
				createTween(cover, TweenInfo.new(0.2, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {BackgroundTransparency = 0})
				wait(0.3)
				tabClone.Visible = true
				createTween(cover, TweenInfo.new(0.2, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {BackgroundTransparency = 1})
			end
		end)
		...
		searchQuery.FocusLost:Connect(function()
			...
			for i, v in pairs(tabClone:GetDescendants()) do
				if v:IsA("Frame") and v.Parent.Parent:IsA("ScrollingFrame") then
					if string.find(string.lower(v.Name), InputText) then
						...
						for i, b in pairs(v.Parent.Parent:GetChildren()) do
						...
						createTween(v.Parent.Parent, TweenPresets.Fast, {CanvasPosition = Vector2.new(0, searchScrollTo + 50)})
						wait(0.1)
			...
			if noResultsFound == true then
				...
				searchQuery.Text = ""
				createTween(searchButton.Icon1, TweenInfo.new(0.07, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Position = UDim2.new(0.8, 0, 0.5, 0)})
				wait(0.07)
				createTween(searchButton.Icon1, TweenInfo.new(0.07, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Position = UDim2.new(0.2, 0, 0.5, 0)})
				wait(0.07)
				createTween(searchButton.Icon1, TweenInfo.new(0.07, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Position = UDim2.new(0.5, 0, 0.5, 0)})
				searchQuery:CaptureFocus()
			end
		end)
		...
		function tabFuncs:NewSection(SectionOptions) -- section
			...
			sectionClone.Description.Text = SectionOptions.Description
			...
			local function handleNilDesc(obj, Options)
			...
			local function updateElementsExistance()
				...
				for i, v in pairs(sectionClone:GetChildren()) do
					if v:IsA("Frame") then
						...
						if deletedOrder == 0 then -- deleted thing is the first one
							...
							elseif ealierElementsExists == true and v.LayoutOrder == 0 then
								task.spawn(function()
									ealierElementsExists = false
									createTween(v.UICorner, TweenPresets.Medium, {CornerRadius = UDim.new(0, 12)})
									createTween(v:FindFirstChild("CornerCover"), TweenPresets.Medium, {BackgroundTransparency = 1})
									wait(0.5)
			...
			function sectionFuncs:NewToggle(ToggleOptions) -- toggle
				local toggleClone
				local toggled = ToggleOptions.CurrentState
				...
				toggleClone.Hitbox.MouseButton1Up:Connect(function()
					createTween(toggleClone, TweenPresets.Fast, {BackgroundColor3 = Color3.fromRGB(60, 60, 60)})
					if toggled == true then
						createTween(toggleClone.Switch, TweenPresets.Fast, {BackgroundColor3 = Color3.fromRGB(80, 80, 80)})
						createTween(toggleClone.Switch.Circle, TweenPresets.Fast, {BackgroundColor3 = Color3.fromRGB(168, 168, 168)})
						createTween(toggleClone.Switch.Circle, TweenPresets.Fast, {Position = UDim2.new(0.3, 0, 0.5, 0)})
						
						toggled = false
					else
						createTween(toggleClone.Switch, TweenPresets.Fast, {BackgroundColor3 = Color3.fromRGB(53, 132, 228)})
						createTween(toggleClone.Switch.Circle, TweenPresets.Fast, {BackgroundColor3 = Color3.fromRGB(255, 255, 255)})
						createTween(toggleClone.Switch.Circle, TweenPresets.Fast, {Position = UDim2.new(0.7, 0, 0.5, 0)})
				...
				function toggleFuncs:Edit(NewToggleOptions) -- edit toggle
					...
					if NewToggleOptions.CurrentState == true then
						...
						toggled = true
					elseif NewToggleOptions.CurrentState == false then
						createTween(toggleClone.Switch, TweenPresets.Fast, {BackgroundColor3 = Color3.fromRGB(80, 80, 80)})
						createTween(toggleClone.Switch.Circle, TweenPresets.Fast, {BackgroundColor3 = Color3.fromRGB(168, 168, 168)})
						createTween(toggleClone.Switch.Circle, TweenPresets.Fast, {Position = UDim2.new(0.3, 0, 0.5, 0)})
			...
			function sectionFuncs:NewTextBox(TextboxOptions) -- textbox
				...
				if TextboxOptions.Callback ~= nil then
					if TextboxOptions.Trigger == "FocusLost" then
						textBoxClone.TextBox.FocusLost:Connect(function()
							onTriggerConnection = TextboxOptions.Callback(textBoxClone.TextBox.Text)
						end)
					elseif TextboxOptions.Trigger == "TextChanged" then
						textBoxClone.TextBox:GetPropertyChangedSignal("Text"):Connect(function()
							onTriggerConnection = TextboxOptions.Callback(textBoxClone.TextBox.Text)
						end)
					else
						textBoxClone.TextBox.FocusLost:Connect(function()
				...
				function textBoxFuncs:Edit(newTextBoxOptions)
					...
					if newTextBoxOptions.Callback ~= nil then
						if newTextBoxOptions.Trigger == "FocusLost" then
							textBoxClone.TextBox.FocusLost:Connect(function()
								onTriggerConnection = newTextBoxOptions.Callback(textBoxClone.TextBox.Text)
							end)
						elseif newTextBoxOptions.Trigger == "TextChanged" then
							textBoxClone.TextBox:GetPropertyChangedSignal("Text"):Connect(function()
								onTriggerConnection = newTextBoxOptions.Callback(textBoxClone.TextBox.Text)
							end)
						else
			...
			function sectionFuncs:NewDropdown(DropdownOptions)
				...
				local function addChoices(v)
					...
					choice.Title:GetPropertyChangedSignal("TextBounds"):Connect(function()
						...
						dropdownClone.Shadow.Position = UDim2.new(dropdownClone.Drop.Position.X.Scale, dropdownClone.Drop.Position.X.Offset, dropdownClone.Drop.Position.Y.Scale, dropdownClone.Drop.Position.Y.Offset -10)
						dropdownClone.Shadow.Size = UDim2.new(0, dropdownClone.Drop.AbsoluteSize.X, 0, dropdownClone.Drop.AbsoluteSize.Y) + UDim2.new(0, 20, 0, 20)
					end)
					...
					choice.Hitbox.MouseButton1Up:Connect(function()
						...
						if choice.Name ~= selected then
							...
							for i, v in pairs(dropdownClone.Drop:GetChildren()) do
								if v:IsA("Frame") then
				...
				for i, v in pairs(DropdownOptions.Choices) do
					addChoices(v)
				end
				...
				function dropdownFuncs:Edit(newDropdownOptions)
					...
					if newDropdownOptions.Choices ~= nil then
						...
						for i, v in pairs(newDropdownOptions.Choices) do
							addChoices(v)
						end
					end
			...
			function sectionFuncs:NewSlider(SliderOptions)
				...
				local descExists = handleNilDesc(slierClone, SliderOptions)
				...
				if SliderOptions.CurrentValue ~= nil then
					...
					createTween(circle, TweenPresets.Fast, {Position = UDim2.new(SliderOptions.CurrentValue / SliderOptions.MinMax[2], 0, circle.Position.Y.Scale, circle.Position.Y.Offset)})
				else
				...
				value:GetPropertyChangedSignal("Value"):Connect(function()
					createTween(circle, TweenPresets.Fast, {Position = UDim2.new(percentage, 0, circle.Position.Y.Scale, circle.Position.Y.Offset)})
				...
				circle.InputBegan:Connect(function(input)
					if input.UserInputType == Enum.UserInputType.MouseButton1 then
						...
						while dragging do
							wait()
							percentage = math.clamp(snap((UIS:GetMouseLocation().X - bar.AbsolutePosition.X) / bar.AbsoluteSize.X, step), 0, 1)
							value.Value = math.round( SliderOptions.MinMax[1] + (percentage * (SliderOptions.MinMax[2] - SliderOptions.MinMax[1])) )
						end
					end
				end)
		...
		function tabFuncs:Edit(NewName) -- edit tab
			tabButtonClone.Title.Text = NewName
			tabButtonClone.Name = NewName
			tabClone.Name = NewName
			for _, v in pairs(TabTable) do
				if v.Name == tabName then
					v.Name = NewName
				end
